...
from alice.physics.square import build_traversal_snake, build_traversal_zigzag
...
class TestSnakeOrder:
    """Tests for `build_traversal_snake`."""

    def test_2x3_lattice(self):
        """Snake order for a 2×3 lattice."""
        ord_map, latt = build_traversal_snake(lx=2, ly=3)
    ...
    def test_total_sites(self, lx, ly, expected_sites):
        ...
        ord_map, latt = build_traversal_snake(lx, ly)
    ...
    def test_1d_chain(self):
        ...
        ord_map, latt = build_traversal_snake(lx=5, ly=1)
...
class TestZigzagOrder:
    """Tests for `build_traversal_zigzag`."""

    def test_2x3_lattice(self):
        """Zigzag order for a 2×3 lattice."""
        ord_map, latt = build_traversal_zigzag(lx=2, ly=3)
    ...
    def test_total_sites(self, lx, ly, expected_sites):
        """Total number of sites equals lx * ly; all indices are unique."""
        ord_map, latt = build_traversal_zigzag(lx, ly)
        assert len(latt) == expected_sites
        assert len(ord_map) == expected_sites
        assert len(set(ord_map.values())) == expected_sites
    ...
    def test_1d_chain(self):
        ...
        ord_map, latt = build_traversal_zigzag(lx=5, ly=1)
        assert ord_map == {(0, 0): 0, (0, 1): 1, (0, 2): 2, (0, 3): 3, (0, 4): 4}
        assert latt == [(0, 0), (0, 1), (0, 2), (0, 3), (0, 4)]

    def test_differs_from_snake_for_2d(self):
        """Zigzag and snake produce different ord_maps for any 2D lattice."""
        snake_map, _ = build_traversal_snake(lx=4, ly=4)
        zigzag_map, _ = build_traversal_zigzag(lx=4, ly=4)
    ...
    def test_all_columns_top_to_bottom(self):
        ...
        ord_map, _ = build_traversal_zigzag(lx=5, ly=4)
        for col in range(5):
