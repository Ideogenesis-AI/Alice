...
"""Example: XTRG finite-temperature thermodynamics of the 1D spinless free-fermion chain.

Computes ρ(β) = e^{-βH} via XTRG for the tight-binding Hamiltonian

    H = -t Σ_i (c†_i c_{i+1} + h.c.) - μ Σ_i n_i

for a spinless fermion chain of length L with open boundary conditions, then
compares log Z(β), the free energy, internal energy, and entropy against the
exact grand-canonical solution at each cooling step.

Single-particle energies (OBC):

    ε_k = -2t cos(kπ / (L+1)),  k = 1, …, L.

Exact grand-canonical log Z (shifted by the chemical potential μ):

    log Z(β) = Σ_k log(1 + exp(-β (ε_k - μ))).
...
Run from the repository root:
    ...
    uv run python examples/examples_xtrg/xtrg_spinless.py --env-cache-dir /tmp/env_cache
    uv run python examples/examples_xtrg/xtrg_spinless.py --checkpoint-dir /tmp/xtrg_ckpt
    uv run python examples/examples_xtrg/xtrg_spinless.py --help
"""
...
def exact_log_z(L: int, t: float, mu: float, beta: float) -> float:
...
def xtrg_spinless(
    ...
    """Run XTRG for the spinless free-fermion tight-binding chain.
    ...
    if verbose:
        ...
        for n, beta in enumerate(summary.betas):
            T = 1.0 / beta
            lz_exact = exact_log_z(L, t, mu, beta)
...
def _parse_args():
    ...
    p = argparse.ArgumentParser(
        description=(
            "XTRG finite-temperature thermodynamics for the 1D spinless free-fermion chain.\n\n"
            "Examples:\n"
            "  uv run python examples/examples_xtrg/xtrg_spinless.py\n"
            "  uv run python examples/examples_xtrg/xtrg_spinless.py --scheme 1s\n"
            "  uv run python examples/examples_xtrg/xtrg_spinless.py --scheme 1sp\n"
    ...
    p.add_argument(
        '--steps', type=int, default=12, metavar='N',
        help='number of cooling steps (default: 12)',
    )
    p.add_argument(
