...
Under the checkpoint directory (`Options.checkpoint_dir`, or cwd):

```text
checkpoint_dir/
  thermal.ckpt           # latest Summary (kept after success)
  progress.ckpt          # latest Artifact while running (removed on success)
  artifacts/
...
- `progress.ckpt` is overwritten each step and deleted when `run()` finishes
...
- [Options](options.md) — `save_artifacts` / `save_artifacts_since`.
